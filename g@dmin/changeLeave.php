<?php 
   ...
   if(!$conn){
      ...
      $status = ucwords(mysqli_real_escape_string($conn,$_POST['leaveStatus']));
      $stmt = mysqli_stmt_init($conn);
      if(mysqli_stmt_prepare($stmt, "UPDATE leaves SET leave_status=? WHERE id_number=? AND created_at=?")) {
